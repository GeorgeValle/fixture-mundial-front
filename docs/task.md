...
- Objetivo follow-up actual: `/posiciones` debe mostrar clasificación histórica desde fase de grupos usando el ranking confiable de mejores terceros para posición 3, sin depender del bracket ni de `/api/matches`.
...
- Estado de validación del follow-up `/posiciones`: runtime WSL verificado con Node/pnpm Linux en `/home/yorch/.nvm/versions/node/v24.14.0/bin`; pasaron `TMPDIR=/tmp TEMP=/tmp TMP=/tmp pnpm test` (41 archivos, 440 tests), `TMPDIR=/tmp TEMP=/tmp TMP=/tmp pnpm run build` y `TMPDIR=/tmp TEMP=/tmp TMP=/tmp pnpm run lint`.
...
- [x] Follow-up PR #31 P2: simplificar badges de terceros para usar ranking confiable de mejores terceros como fuente principal.
